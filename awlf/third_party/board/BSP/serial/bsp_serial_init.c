...
void bsp_serial_pre_init(bsp_serial_t bsp_serial)
{
    ...
    is_enable_int = device_get_regparams(&bsp_serial->parent.parent) &
                    (SERIAL_REG_DMA_RX | SERIAL_REG_DMA_TX | SERIAL_REG_INT_RX | SERIAL_REG_INT_TX);
    switch ((uint32_t)bsp_serial->handle.Instance)
    {
#ifdef USE_SERIAL_1
        case USART1_BASE: _bsp_serial1_pre_init(bsp_serial, is_enable_int); break;
#endif

#ifdef USE_SERIAL_3
        case USART3_BASE: _bsp_serial3_pre_init(bsp_serial, is_enable_int); break;
#endif

#ifdef USE_SERIAL_6
        case USART6_BASE: _bsp_serial6_pre_init(bsp_serial, is_enable_int); break;
#endif
    }
}
