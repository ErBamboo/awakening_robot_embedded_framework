...
Device_t device_find(char* name)
{
    ...
    if (!name)
        return NULL;
    primask = aw_hw_disable_interrupt();
...
AwlfRet_e device_register(Device_t Dev, char* name, uint32_t regparams)
{
    ...
    intLevle = aw_hw_disable_interrupt();
    if (device_find(name))
    {
        /* 如果设备名冲突，直接返回 */
        // TODO: log
        aw_hw_restore_interrupt(intLevle);
...
{
    while (!Dev || !Dev->interface || !data || !len)
    {
    }; // TODO: assert
...
{
    AwlfRet_e ret = AWLF_OK;
    while (!Dev || !Dev->interface || !Dev->interface->close)
    {
    }; // TODO: assert
    if (!device_check_status(Dev, DEV_STATUS_OPENED))
    {
