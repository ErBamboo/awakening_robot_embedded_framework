...
static AwlfRet_e bsp_serial_configure(HalSerial_t serial, SerialCfg_t cfg)
{
    ...
    switch (cfg->stopBits)
    {
        case STOP_BITS_1: huart->Init.StopBits = UART_STOPBITS_1; break;
        case STOP_BITS_2: huart->Init.StopBits = UART_STOPBITS_2; break;
        default: // TODO: assert
            break;
    }
    switch (cfg->parity)
    {
        case PARITY_NONE: huart->Init.Parity = UART_PARITY_NONE; break;
        case PARITY_ODD: huart->Init.Parity = UART_PARITY_ODD; break;
        case PARITY_EVEN: huart->Init.Parity = UART_PARITY_EVEN; break;
    }
    switch (cfg->flowCtrl)
    {
        case FLOW_CTRL_NONE: huart->Init.HwFlowCtl = UART_HWCONTROL_NONE; break;
        case FLOW_CTRL_RTS: huart->Init.HwFlowCtl = UART_HWCONTROL_RTS; break;
        case FLOW_CTRL_CTS: huart->Init.HwFlowCtl = UART_HWCONTROL_CTS; break;
        case FLOW_CTRL_CTS_RTS: huart->Init.HwFlowCtl = UART_HWCONTROL_RTS_CTS; break;
    }
