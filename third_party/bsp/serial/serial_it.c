...
static void _bsp_serial_rxdma_err_cb(DMA_HandleTypeDef* _hdma)
{
    bsp_serial_t bsp_serial;
    bsp_serial = (bsp_serial_t)_hdma->Parent;
...
void bsp_serial_dma_cfg(bsp_serial_t bsp_serial, uint32_t dma_regparams)
{
    ...
    else if (dma_regparams == SERIAL_REG_DMA_RX)
    {
        ...
        while (!bsp_serial->rx_multibuf) {}; // TODO: assert
    ...
    if (dma_regparams == SERIAL_REG_DMA_RX)
    {
        ...
        {
            HAL_UARTEx_ReceiveToIdle_DMA(&bsp_serial->handle, bsp_serial->rx_multibuf->container0,
                                         bsp_serial->rx_multibuf->container_len);
        }
        else
        {
            // TODO： 这一块导致整个函数过长，未来将其打包放进一个新的函数里面去，专门负责配置DMA多buf，更符合单一责任原则
            ...
            */
            ...
            ret = HAL_DMAEx_MultiBufferStart_IT(hdma,
                                                ...
                                                (uint32_t)bsp_serial->rx_multibuf->container1,
                                                bsp_serial->rx_multibuf->container_len);
            while (ret != HAL_OK) {}; // TODO: assert
        }
    }
}
